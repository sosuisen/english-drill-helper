...
- [x] 停止すると、一覧のその行の最終再生日時が更新される（replaceDrill はテストなしで書いた実装。テストで裏付ける）
- [x] 同じドリルを2回続けて停止しても、2回目も行が更新される（行の検索をオブジェクト同一性ではなく指紋で行う）
- [x] lastPlayedAtProperty() は行に日時が付いたため役割が重なる。削除した
